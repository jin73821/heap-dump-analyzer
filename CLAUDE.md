...
**요약 탭은 ≥1400px 에서만 2열**(`body.cd-analyze` 스코프, 컨테이너 1600px + `.cd-sum-cols` flex — AI 좌·콜 체인 우) 이고 그 미만은 세로 1열 그대로다. 2열 래퍼는 **grid 고정 2열이 아니라 flex** 여야 한다(콜 체인 미렌더 시 AI 가 전폭을 쓰도록 — grid 면 빈 칸이 남는다). 전폭이 옳은 카드(히어로·덤프 메타·경고 배너)는 래퍼 밖에 둘 것. 회귀 방어 `CoreDumpPdfReportServiceTest`(6)/`CoreDumpPrintTemplateSmokeTest`(5, PDF `%PDF` 바이트 스모크 포함)/`CoreDumpReportEndpointTest`(7, standalone MockMvc — 라우팅·헤더·rev 분기·404/400).
