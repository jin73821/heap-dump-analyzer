...
public class RagService {
    ...
    public String fetchContextForLlm(String query) {
        ...
        // ⚠ 이 경고가 없으면 모델이 참고 자료의 수치를 "현재 분석 대상"의 것으로 단정한다.
        //   2026-08-29 실제 사례: 다른 분석 건의 "byte[] 17,574개 / 16.18MB" 를 현재 덤프 수치로 제시해,
        //   같은 답변 안에서 진짜 현재 덤프 값("byte[] 347개 / 74.45MB")과 모순되는 두 수치가 공존했다.
        //   컨텍스트에는 현재 덤프 분석 결과와 이 자료가 나란히 붙으므로 모델 입장에선 구분할 근거가 없다.
        sb.append("⚠ 이 자료는 **다른 분석 건**의 사례·수치일 수 있습니다. 여기 나온 수치·클래스명·파일명을 "
                + "현재 분석 대상의 것으로 단정하지 마세요. 인용할 때는 참고 사례임을 밝히고, 현재 덤프의 수치는 "
                + "위의 분석 결과 섹션만 근거로 삼으세요.\n\n");
