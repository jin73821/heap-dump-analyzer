#ps -ef | grep heap-analyzer-2.0.6.jar | grep -v grep | awk '{print "kill -15 " $2}' | sh;
#nohup java -jar /opt/genspark/webapp_dump/target/heap-analyzer-2.0.6.jar --server.port=18080 &
...
if [ -n "$RUNNING_PIDS" ]; then
    ...
    for i in $(seq 1 20); do
        sleep 1
        REMAINING=$(ps -ef | grep heap-analyzer-2.0.6.jar | grep -v grep | awk '{print $2}')
        if [ -z "$REMAINING" ]; then
