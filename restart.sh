#ps -ef | grep heap-analyzer-2.1.6.jar | grep -v grep | awk '{print "kill -15 " $2}' | sh;
#nohup java -jar /opt/genspark/webapp_dump/target/heap-analyzer-2.1.6.jar --server.port=18080 &

JAR=/opt/genspark/webapp_dump/target/heap-analyzer-2.1.6.jar
...
RUNNING_PIDS=$(ps -ef | grep heap-analyzer-2.1.6.jar | grep -v grep | awk '{print $2}')
if [ -n "$RUNNING_PIDS" ]; then
    echo "[restart] SIGTERM 전송: PID=$RUNNING_PIDS — 종료 대기 중..."
    kill -15 $RUNNING_PIDS 2>/dev/null
    for i in $(seq 1 20); do
        sleep 1
        REMAINING=$(ps -ef | grep heap-analyzer-2.1.6.jar | grep -v grep | awk '{print $2}')
        if [ -z "$REMAINING" ]; then
