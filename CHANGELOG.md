...
**결과:** JS·템플릿 0건(1차 가드 유지). **Java 69건.** 1차 가드가 JS·템플릿만 스캔하고 있었다.
- A 1: `ChromaSearchService` 색인 현황 — count 조회가 실패하면 상한에 닿아도 `partial=false` 로 과소보고했다. 스캔 건수로 추정하고 warn 을 남긴다.
- W 13 (`logger.warn`):
  - MAT·OOM·SysProp 임시 디렉토리 정리 실패 6.
  - settings.json 숫자 설정 손상 5.
  - sysroot 부분 산출물 정리 실패 1.
  - 계정 신청 승인 시 알 수 없는 권한 문자열이 USER 로 승인되는 경우 1(동작 유지, 기록만).
- B 52 (`logger.debug`): 클라이언트 disconnect 후 SSE 전송, 선택적 파싱, 비 Linux `/proc`, 프로세스 종료 후 출력 읽기 등 정당한 폴백.
- C 3: `HeapReportApiController` — 모든 예외를 이미 흡수하는 `send*Error` 헬퍼를 감싼 죽은 try 를 제거했다.
- 신규 로거: `SecurityConfig`·`AccountRequestController`·`SseJson`.

**가드 (`EmptyCatchGuardTest`, 2건 유지):**
- 스캔 대상에 `src/main/java` 를 추가했다.
- 바인딩 생략 `catch { }` 와 `;` 만 있는 본문도 빈 블록으로 판정한다.
- 스캐너 자체 검증을 12 → 20 형태로 늘렸다.
- 임시 Java 파일에 빈 catch 를 넣어 가드가 실패하는 것까지 확인했다.

**Files 페이지:** GC 로그 분류 이동 안내·삭제 확인 문구의 "AI 해석" → "AI 분석"(결과 페이지 용어와 통일).

**검증:** 조치 후 스캔 0건, `mvn test` 775 통과(라이브 4 skip), 운영 재기동 정상(기동 중 신규 warn 0건).

**저장소 밖 잔여:** `/opt/chroma/app/indexer.py` 의 `except Exception: pass` 1건 — Chroma 색인기 트리라 손대지 않았다.

## [2026-09-14] GC 로그 결과 화면 — AI 분석 카드 재디자인 · 분석 확인 모달 · 경과 시간 · 소견 좌측 색 띠 제거 (v2.5.2 유지)

**대상:** `templates/gc-log/analyze.html` · `static/js/gc-log-analyze.js` · `static/css/gc-log.css` · `GcLogTemplateSmokeTest`

1. **문구:** 결과 화면의 `AI 해석` → `AI 분석`(상단 버튼·카드 제목·안내·삭제 확인·오류 문구).
   - Files 페이지의 GC 로그 분류·삭제 안내에 남은 "AI 해석" 2곳은 범위 밖이라 그대로 두었다.
2. **AI 분석 카드 스타일:**
   - 심각도 행: 배지 + 근거 문구, 옅은 배경 + 같은 계열 1px 테두리.
   - 블록 4종(요약·근본 원인·권고·GC 튜닝 제안)은 1px 테두리 카드에 색 아이콘 제목을 붙였다. 요약은 한 단계 큰 글씨.
   - 권고는 번호 원형 배지 목록. ≥1100px 에서는 권고와 GC 튜닝 제안을 2열로 두고, 하나만 있으면 전폭.
   - JVM 옵션(`-XX:…`·`-Xmx4g`·`-Xlog:…`·`-Xloggc:…`)은 코드 칩이다. 이스케이프한 뒤에 감싸고, 값 문자에 `&` 를 넣지 않아 엔티티를 가르지 않는다. 칩은 inline-block 이라 옵션 안의 `-` 에서 줄이 끊기지 않는다.
   - 메타 줄: 모델 · 분석 시각 · **소요**(`latencyMs`) · 삭제.
3. **분석 버튼 + 확인 모달:**
   - 카드 제목 오른쪽에 `분석` 버튼을 두었다. 결과가 있으면 `재분석`, 실행 중이면 `분석 중…` 이다.
   - 상단 `AI 분석` 버튼과 카드 버튼 모두 확인 모달(`#aiConfirmModal`, 골격 `.gcl-modal` — 함정 17)만 연다. LLM 요청은 모달의 `분석 시작` 에서만 나간다.
   - 모달 내용: 파일명, 연결된 힙 덤프가 있으면 함께 보낸다는 안내, API 비용·15~40초 안내. 재분석이면 "기존 결과가 대체된다" 안내를 더한다.
   - Esc·바깥 클릭으로 닫히고, 닫으면 포커스를 트리거 버튼으로 되돌린다.
4. **분석 중 표시:** 스피너 + 제목·설명 + **경과 N초**(1초 단위, tabular-nums) + 불확정 진행 막대. 모션 감소 설정이면 애니메이션을 끈다.
   - ≤640px 에서는 경과 알약이 설명 아래로 내려간다.
   - 성공·실패 모두 타이머를 정리한다. 성공하면 토스트로 소요 초를 알린다.
   - **실패하면 오류를 위에 얹고 직전 결과는 그대로 보인다.** 종전엔 오류 한 줄만 남아 DB 에 남은 옛 결과가 사라진 것처럼 보였다.
   - 세션 활동 가드를 분석마다 새로 등록하던 누적을 1회 등록으로 고쳤다.
5. **소견 카드:** 좌측 4px 색 띠(`border-left` + 심각도별 `border-left-color`)를 지웠다.
   - 심각도는 옅은 배경 + 같은 계열 1px 테두리 + 배지로 드러낸다.
   - 같은 이유로 권고 줄의 좌측 2px 선도 위쪽 점선 구분선으로 바꿨다.
   - `gc-log.css` 에는 이제 `border-left` 선언이 없다.

**검증:**
- `GcLogTemplateSmokeTest` 기존 메서드에 단언을 추가했다.
  - 템플릿: `AI 해석` 부재, 두 버튼·모달 골격.
  - JS: `runAi` 는 모달만 열고 `/ai-analyze` 를 부르지 않음, 확인 → `startAi`, 경과 초, 타이머 정리, 가드 1회, 블록 4종.
  - CSS: 주석을 뺀 `gc-log.css` 에 `border-left` 없음.
- 정적 픽스처(실제 CSS·JS + fetch 스텁)를 헤드리스 Chrome 으로 찍었다. 상태는 결과·재분석 모달·분석 중(경과 4초)·호출량 제한 오류, 폭은 1440/500px 이다. 500px 에서 가로 넘침이 없음을 확인했다.
- 테스트 775 전체 통과(라이브 4 skip), 운영 재기동 후 정적 리소스 반영 확인.
